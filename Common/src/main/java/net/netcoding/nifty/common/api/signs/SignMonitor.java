...
import net.netcoding.nifty.common.minecraft.event.block.piston.BlockPistonRetractEvent;
import net.netcoding.nifty.common.minecraft.event.entity.EntityExplodeEvent;
...
public class SignMonitor {
	...
	public SignMonitor(MinecraftPlugin plugin) {
		this.signListener = new MinecraftSignListener(plugin);
	}
	...
	private class MinecraftSignListener extends MinecraftListener {

		public MinecraftSignListener(MinecraftPlugin plugin) {
			super(plugin);
		}
		...
		public void onProfileJoin(PlayerJoinEvent event) {
			...
			if (IS_POST_1_9_3) {
				...
				for (NbtCompound compound : compounds) {
					...
					for (Map.Entry<SignListener, List<String>> entry : SignMonitor.this.listeners.entrySet()) {
						for (String line : signInfo.getLines())
							entry.getValue().stream().filter(key -> line.toLowerCase().contains(key)).forEach(key -> SignMonitor.this.sendSignUpdate(player, key, signInfo));
					}
				}
			}
		}
