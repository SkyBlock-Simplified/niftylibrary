...
import net.netcoding.niftybukkit._new_.api.inventory.NbtKeys;
import net.netcoding.niftybukkit._new_.api.inventory.events.FakeItemInteractEvent;
...
import static net.netcoding.niftybukkit._new_.api.inventory.FakeInventory.getClickedItem;
				...
				if (InventoryType.SlotType.OUTSIDE != event.getSlotType()) {
					if (FakeItem.this.getItemOpener() != null) {
						...
						if (!FakeInventory.isOpenAnywhere(profile)) {
							ItemStack firstClickItem = getClickedItem(event);
							ItemStack placeClickItem = getClickedItem(event, false);
	...
	private static class ItemOpenerListener extends BukkitListener {
		...
		public void onInventoryCreative(InventoryCreativeEvent event) {
			...
			if (isAnyItemOpener(ItemStack)) {
				event.setCancelled(true);
				event.setResult(net.netcoding.niftybukkit._new_.minecraft.event.Event.Result.DENY);
			}
		}
